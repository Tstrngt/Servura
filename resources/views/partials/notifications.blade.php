...
<div class="{{ $wrapperClass }}" x-data="{
    ...
    <button @click="open = !open" type="button" class="relative inline-flex items-center {{ $bellClass }} focus:outline-none">
        ...
        <span x-show="count > 0" x-text="count" class="absolute -top-1 -right-1 flex h-4 w-4 items-center justify-center rounded-full bg-red-500 text-xs font-bold text-white ring-2 ring-white"></span>
    </button>

    <div x-show="open" x-transition @click.outside="open = false" class="absolute {{ $dropdownClass }} z-50 rounded-lg bg-white shadow-lg ring-1 ring-black ring-opacity-5 overflow-hidden">
