                    ...
                    <form action="{{ route('admin.services.update', $service) }}" method="POST" x-data="{ activeTab: 'general' }" @invalid.capture="activeTab = $event.target.closest('[data-tab]')?.dataset.tab || activeTab">
                        ...
                        <div class="grid grid-cols-1 gap-6 md:grid-cols-2 xl:grid-cols-4 mb-6">
                            ...
                            <div class="form-group">
                                ...
                                <select id="fulfillment_type" name="fulfillment_type" class="form-input" required>
                                    <option value="manual" {{ old('fulfillment_type', $service->fulfillment_type) === 'manual' ? 'selected' : '' }}>Handmatig met ticket</option>
                                    <option value="directadmin" {{ old('fulfillment_type', $service->fulfillment_type) === 'directadmin' ? 'selected' : '' }}>Automatisch via DirectAdmin</option>
                                </select>
                            </div>
                            <div class="form-group">
                            ...
                            <section x-cloak x-show="activeTab === 'pricing'" data-tab="pricing">
                                ...
                                <div class="grid grid-cols-1 gap-3 sm:grid-cols-2">
                                    @foreach($billingCycles as $cycle => $label)
                                        @php
                                            $savedPrice = $service->prices->firstWhere('billing_cycle', $cycle);
                                            $amount = old("prices.$cycle.amount", $savedPrice?->price);
                                            $enabled = old("prices.$cycle.enabled", (bool) $savedPrice?->is_enabled);
                                        @endphp
                                        <div x-data="{ amount: @js($amount) }" class="rounded-xl bg-slate-50 p-3 ring-1 ring-slate-200">
                                            <label class="mb-2 flex items-center gap-2 text-sm font-semibold text-slate-800">
                                                ...
                                                <input type="checkbox" name="prices[{{ $cycle }}][enabled]" value="1" {{ $enabled ? 'checked' : '' }} class="rounded border-slate-300 text-primary-600">
                                                {{ $label }}
                                            </label>
                        ...
                        <div class="mb-6 grid grid-cols-1 gap-3 sm:grid-cols-2 xl:grid-cols-4">
                            ...
                            <label class="flex items-center rounded-xl bg-amber-50 px-4 py-3 ring-1 ring-amber-200">
                                <input type="checkbox" name="priority_support" value="1" {{ old('priority_support', $service->priority_support) ? 'checked' : '' }} class="rounded border-amber-300 text-amber-600 shadow-sm focus:border-amber-300 focus:ring focus:ring-amber-200 focus:ring-opacity-50">
