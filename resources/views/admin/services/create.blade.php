        ...
        <div>
            <div class="overflow-hidden rounded-2xl bg-white shadow-sm ring-1 ring-slate-200">
                <div class="p-5 sm:p-6 lg:p-8">
                    <form action="{{ route('admin.services.store') }}" method="POST" x-data="{ activeTab: 'general' }" @invalid.capture="activeTab = $event.target.closest('[data-tab]')?.dataset.tab || activeTab">
                        @csrf
                        ...
                        <div class="grid grid-cols-1 gap-6 md:grid-cols-2 xl:grid-cols-4 mb-6">
                            ...
                            <div class="form-group">
                                ...
                                <select id="server_connection_id" name="server_connection_id" class="form-input">
                                    ...
                                    @foreach($serverConnections as $connection)<option value="{{ $connection->id }}" {{ old('server_connection_id') == $connection->id ? 'selected' : '' }}>{{ $connection->name }} · {{ $connection->provider_label }}</option>@endforeach
                                </select>
                            ...
                            <div class="form-group">
                                ...
                                <input type="text" id="provider_package" name="provider_package" class="form-input" value="{{ old('provider_package') }}" placeholder="Bijvoorbeeld: hosting_starter">
                        ...
                        <div x-cloak x-show="activeTab === 'features'" data-tab="features" x-data="popupEditor(@js(['badges' => old('popup_badges', []), 'details' => old('popup_details', [])]))" class="mb-8 rounded-2xl border border-slate-200 bg-slate-50/70 p-5 sm:p-6">
